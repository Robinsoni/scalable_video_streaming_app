...
const mp4FileName = 'WIN_20240106_15_38_55_Pro.mp4';
...
const s3ToS3 = async (mp4FileName) => {
   ...
   try {
       ...
       for (const { resolution, videoBitrate, audioBitrate } of resolutions) {
           ...
           const outputFileName = `${mp4FileName.replaceAll(
               '.',
               '_'
           ).replaceAll(" ","_")}_${resolution}.m3u8`;
           const segmentFileName = `${mp4FileName.replaceAll(
               '.',
               '_'
           ).replaceAll(" ","_")}_${resolution}_%03d.ts`;
           await new Promise((resolve, reject) => {
               ffmpeg('./local.mp4')
                   .outputOptions([
                       `-c:v h264`,
       ...
       const masterPlaylistFileName = `${mp4FileName.replaceAll(
        '.',
        '_'
    ).replaceAll(" ","_")}_master.m3u8`;
       ...
       for (const file of files) {
           if (!file.startsWith(mp4FileName.replaceAll(
            '.',
            '_'
        ).replaceAll(" ","_"))) {
               continue;
